...
/* harmony export */   "default": () => (/* binding */ loadCalendar)
/* harmony export */ });
/* harmony import */ var _images_flexible_stay_calendar_icon_jpg__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./images/flexible-stay-calendar-icon.jpg */ "./src/images/flexible-stay-calendar-icon.jpg");
/* harmony import */ var _images_flexible_stay_calendar_icon_selected_jpg__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./images/flexible-stay-calendar-icon-selected.jpg */ "./src/images/flexible-stay-calendar-icon-selected.jpg");
...
var monthNames = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December']; //generating the calander only trigger when show calander clicked
...
function loadCalendar() {
  ...
  function passDatesUpToStringDisplay() {
    if (window.searchBarSelected.placesToStay) {
      if (!storeDateSelection.checkIn.dateString) {
        ...
        $('#check-in-string-date-display').empty().append("".concat(storeDateSelection.checkIn.dayDate, " ").concat(monthNames[storeDateSelection.checkIn.month].substring(0, 3)));
      ...
      if (!storeDateSelection.checkOut.dateString) {
        ...
        $('#check-out-string-date-display').empty().append("".concat(storeDateSelection.checkOut.dayDate, " ").concat(monthNames[storeDateSelection.checkOut.month].substring(0, 3)));
    ...
    if (window.searchBarSelected.experiences) {
      ...
      if (storeDateSelection.checkIn.dateString) {
        $('.experience-check-in-text').append("".concat(storeDateSelection.checkIn.dayDate, " ").concat(monthNames[storeDateSelection.checkIn.month].substring(0, 3))).show();
      ...
      if (storeDateSelection.checkOut.dateString) {
        $('.experience-check-out-text').prepend("".concat(storeDateSelection.checkOut.dayDate, " ").concat(monthNames[storeDateSelection.checkOut.month].substring(0, 3))).show();
  ...
  function handleSelectedCalendarDate() {
    if (window.searchBarSelected.placesToStay) {
      var checkInSelected = $('.search-bar-content-selected').hasClass('check-in-container');
      var checkOutSelected = $('.search-bar-content-selected').hasClass('check-out-container');
  ...
  var HandleDateSeletion = /*#__PURE__*/function () {
    function HandleDateSeletion() {
      _classCallCheck(this, HandleDateSeletion);

      _defineProperty(this, "storedDate", null);

      _defineProperty(this, "year", null);

      _defineProperty(this, "month", null);

      _defineProperty(this, "dayDate", null);

      _defineProperty(this, "absoluteTime", null);

      _defineProperty(this, "$dateDomElement", null);
    }

    _createClass(HandleDateSeletion, [{
      key: "dateString",
      get: function get() {
      ...
      */
      ...
      value: function dateSelected() {
        ...
        this.$dateDomElement = $("[data-date='".concat(this.dateString, "']"));
        ...
        var _this$dateString$spli = this.dateString.split('-'),
            _this$dateString$spli2 = _slicedToArray(_this$dateString$spli, 3),
            year = _this$dateString$spli2[0],
            month = _this$dateString$spli2[1],
            date = _this$dateString$spli2[2];
    ...
    return HandleDateSeletion;
  }(); // handle checkin and checkout dates


  var storeDateSelection = {
  ...
  storeDateSelection.checkOut.handleDateOrder = function () {
    if (this.absoluteTime < storeDateSelection.checkIn.absoluteTime) {
      var selectedDate = this.dateString;
  ...
  function highlightDaysWithinStay() {
      ...
      $('.future-calendar-square').on({
        'mouseover': function mouseover() {
          if (storeDateSelection.checkIn.dateString && storeDateSelection.checkIn.absoluteTime < new Date($(this).data('date')).getTime()) {
            ...
            storeDateSelection.checkIn.$dateDomElement.nextUntil(this, '.future-calendar-square').addClass('hover-highlight-days-within-stay'); // handle highlight while the selected month and hover are over two months 

            var _$$data$split = $(this).data('date').split('-'),
                _$$data$split2 = _slicedToArray(_$$data$split, 2),
                hoverMonth = _$$data$split2[1];

            if (storeDateSelection.checkIn.month < hoverMonth) {
              $("[data-date='".concat(storeDateSelection.checkIn.year, "-").concat(hoverMonth, "-1']")).prev().nextUntil(this, '.future-calendar-square').addClass('hover-highlight-days-within-stay');
            }
            ...
            $(this).nextUntil(storeDateSelection.checkOut.$dateDomElement, '.future-calendar-square').addClass('hover-highlight-days-within-stay'); // handle highlight while the selected month and hover are over two months

            var _$$data$split3 = $(this).data('date').split('-'),
                _$$data$split4 = _slicedToArray(_$$data$split3, 2),
                _hoverMonth = _$$data$split4[1];

            if (_hoverMonth < storeDateSelection.checkOut.month) {
              $("[data-date='".concat(storeDateSelection.checkOut.year, "-").concat(storeDateSelection.checkOut.month, "-1']")).prev().nextUntil(storeDateSelection.checkOut.$dateDomElement, '.future-calendar-square').addClass('hover-highlight-days-within-stay');
            }
          }
        },
    ...
    if (storeDateSelection.checkIn.dateString && storeDateSelection.checkOut.dateString) {
      storeDateSelection.checkIn.$dateDomElement.nextUntil(storeDateSelection.checkOut.$dateDomElement, '.future-calendar-square').addClass('selected-highlight-days-within-stay');

      if (storeDateSelection.checkIn.month < storeDateSelection.checkOut.month) {
        $("[data-date='".concat(storeDateSelection.checkOut.year, "-").concat(storeDateSelection.checkOut.month, "-1']")).prev().nextUntil(storeDateSelection.checkOut.$dateDomElement, '.future-calendar-square').addClass('selected-highlight-days-within-stay');
      }
  ...
  (function calenderOpen() {
    ...
    function generateCalender() {
      var selectedMonth = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : currentMonth;
      var selectedYear = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : currentYear;
      $('.calendar-day-square').remove();
      $('.calendar-square-blanks').remove();

      var createCalenderForMonth = function createCalenderForMonth(month, year, $elementForMonth) {
        var daysInMonth = new Date(year, month + 1, 0).getDate();
        var startDayOfMonth = new Date(year, month, 1).getDay();
        startDayOfMonth = startDayOfMonth === 0 ? 7 : startDayOfMonth;

        for (var i = 1; i < startDayOfMonth; i++) {
          $elementForMonth.append("<div class=\"calendar-square-blanks\"></div>");
        } // monthRelaiveToNow  past = -1, currentMonth = 0, future = 1


        var appendCalendarDates = function appendCalendarDates(monthRelativeToNow) {
          var futureDateClass = monthRelativeToNow === 1 ? 'future-calendar-square' : null;

          for (var _i2 = 1; _i2 <= daysInMonth; _i2++) {
            if (monthRelativeToNow === 0) {
              futureDateClass = _i2 >= currentDate ? 'future-calendar-square' : null;
            }

            $elementForMonth.append("<div data-date=\"".concat(year, "-").concat(month, "-").concat(_i2, "\" class=\"calendar-day-square ").concat(futureDateClass, "\">").concat(_i2, "</div>"));
          }
        };
      ...
      createCalenderForMonth(selectedMonth + 2, selectedYear, $('.overflow-next-month-calendar-numbers')); // highlight previously selected dates

      if (storeDateSelection.checkIn.dateString) {
        storeDateSelection.checkIn.$dateDomElement = $("[data-date='".concat(storeDateSelection.checkIn.dateString, "']"));
        storeDateSelection.checkIn.$dateDomElement.addClass('date-selected');
      }

      if (storeDateSelection.checkOut.dateString) {
        storeDateSelection.checkOut.$dateDomElement = $("[data-date='".concat(storeDateSelection.checkOut.dateString, "']"));
        storeDateSelection.checkOut.$dateDomElement.addClass('date-selected');
      }

      highlightDaysWithinStay(); // reset position to 0 after animation

      $('.displayed-month-numbered-dates').css('left', '0');
      $('.month-title').css('left', '0'); // Title for each month

      var monthYearTitle = function monthYearTitle(month, year) {
        if (month > 11) {
          return "".concat(monthNames[month - 12], " ").concat(year + 1);
        }
      ...
      $('.overflow-next-month-title').empty().append(monthYearTitle(selectedMonth + 2, selectedYear)); // event listener for calendar square on click
    ...
    function nextMonth() {
      ...
      $.when($('.month-title').animate({
        left: "-389px"
      }, 200), $('.displayed-month-numbered-dates').animate({
        left: "-389px"
      }, 200)).then(function () {
        generateCalender(selectedMonth, selectedYear);
      }).done(function () {
...
function calendarFlexibleOrSpecificDates() {
  ...
  $('.dates-specific-dates').on('click', function () {
    ...
    $('.flexible-dates-container').hide();
    $('.calendar-months-container-stays').show(); // search bar content change

    $('.check-in-container').show();
    $('.check-out-container').show();
    $('.search-bar-flexible-dates-container').removeClass('search-bar-flexible-dates-container-display').removeClass('search-bar-content-selected'); // select check in container if no container selected
  ...
  $('.dates-flexible-dates').on('click', function () {
    ...
    $('.flexible-dates-container').show(); // search bar content change

    $('.check-in-container').hide();
    $('.check-out-container').hide();
    $('.search-bar-flexible-dates-container').addClass('search-bar-flexible-dates-container-display').addClass('search-bar-content-selected'); //
  ...
  (function flexibleDisplayMonthOptions() {
    ...
    for (var i = 0; i < 6; i++) {
      ...
      $(".flexible-dates-month-title:eq(".concat(i, ")")).append("<span class='flexible-month-short-string-".concat(i, "'>").concat(monthNames[currentMonth + i], "</span>"));

      if (i < 2) {
        $('#flexible-months-of-stay').append("<span id='flexible-month-short-string-".concat(i, "'>").concat(monthNames[currentMonth + i].substring(0, 3), "<span class=\"comma\">,</span> </span>"));
      } else {
        $('#flexible-months-of-stay').append("<span id='flexible-month-short-string-".concat(i, "'>").concat(monthNames[currentMonth + i].substring(0, 3), "<span class=\"comma\">,</span> </span>"));
        $("#flexible-month-short-string-".concat(i)).hide();
      }
    }
  })();
  ...
  $('.flexible-dates-month-option').on('click', function () {
    if ($(this).hasClass('flexible-dates-month-option-selected')) {
      ...
      $(this).removeClass('flexible-dates-month-option-selected').find('img').attr('src', _images_flexible_stay_calendar_icon_jpg__WEBPACK_IMPORTED_MODULE_0__);
      $("#".concat($(this).find('span').attr('class'))).hide();
      removeCommaLastMonthElement();
      return;
    }

    $(this).addClass('flexible-dates-month-option-selected').find('img').attr('src', _images_flexible_stay_calendar_icon_selected_jpg__WEBPACK_IMPORTED_MODULE_1__);
    $("#".concat($(this).find('span').attr('class'))).show();
    removeCommaLastMonthElement();
  });
...
/***/ "./src/images/flexible-stay-calendar-icon-selected.jpg":
/*!*************************************************************!*\
  !*** ./src/images/flexible-stay-calendar-icon-selected.jpg ***!
  \*************************************************************/
...
"use strict";
module.exports = __webpack_require__.p + "979d3fc113a2234037cf.jpg";

/***/ }),

/***/ "./src/images/flexible-stay-calendar-icon.jpg":
/*!****************************************************!*\
  !*** ./src/images/flexible-stay-calendar-icon.jpg ***!
  \****************************************************/
/***/ ((module, __unused_webpack_exports, __webpack_require__) => {
...
"use strict";
module.exports = __webpack_require__.p + "35fd3755ca7dbff2a3c5.jpg";
...
/* harmony import */ var _calendar__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./calendar */ "./src/calendar.js");
/* harmony import */ var _handleResize__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./handleResize */ "./src/handleResize.js");
/* provided dependency */ var $ = __webpack_require__(/*! jquery */ "./node_modules/jquery/dist/jquery.js");
function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }
...
$(function () {
  ...
  window.searchBarSelected = {
    ...
    set placesToStay(open) {
      ...
      if (window.calendarLoaded) {
        _calendar__WEBPACK_IMPORTED_MODULE_1__.default.passDatesUpToStringDisplay();
      }
  ...
  $('.nav-search-options').on('click', function () {
    ...
    $(this).addClass('selected-nav-search-option'); // display content for selected section
    ...
    if (this.id === 'online-experiences') {//$('#search-trip-bar-stays').addClass('selected-search-bar-content');
    }
  ...
  $('.search-bar-content').on('click', function () {
    ...
    function closeSearchBarOptions(event) {
      ...
      if (!$(event.target).closest($('.search-trip-bar')).length > 0) {
        ...
        $('.search-bar-content-selected').removeClass('search-bar-content-selected'); // remove event listener
  ...
  function guestOptions() {
    var typeOfGuests = ['adults', 'children', 'infants'];

    var Guests = /*#__PURE__*/function () {
      function Guests(type) {
        _classCallCheck(this, Guests);

        _defineProperty(this, "numberOf", 0);
      ...
      _createClass(Guests, [{
        ...
        set: function set(number) {
          ...
          $(".number-of-".concat(this.type)).empty().append(number);

          if (this.numberOf === 0) {
            $(".reduce-number-of-".concat(this.type)).addClass('inactive-at-zero');
          }

          if (this.numberOf > 0) {
            $(".reduce-number-of-".concat(this.type)).removeClass('inactive-at-zero');
          }
    ...
    function totalGuestsDisplayString() {
      ...
      var adultChildrenTotal = guestTypes.adults.amount + guestTypes.children.amount;
      ...
      $('#guest-remove-button').addClass('display-delete-button').on('click', function () {
      ...
      if (guestTypes.infants.amount === 1) infantsString = ", ".concat(guestTypes.infants.amount, " infant");
      if (guestTypes.infants.amount > 1) infantsString = ", ".concat(guestTypes.infants.amount, " infants");
      $('.guest-container-total-amount-string').empty().append("".concat(adultChildrenTotal, " ").concat(adultChildrenTotal === 1 ? 'guest' : 'guests').concat(infantsString));
    }

    var guestTypes = {};
    typeOfGuests.forEach(function (type) {
      guestTypes[type] = new Guests(type);
      $(".reduce-number-of-".concat(type)).on('click', function reduceNumberOf() {
        if (guestTypes[type].amount === 0) return;
        guestTypes[type].amount -= 1;
      });
      $(".increase-number-of-".concat(type)).on('click', function increaseNumberOf() {
        guestTypes[type].amount += 1;
      });
    });
  }
  ...
  (function listOfDestinations() {
    var historicDestinations = [{
      ...
      location: "Brittany",
      region: "Bourgogne-Franche-Comt\xE9"
    }, {
      location: "Budapest",
      region: "Hungary"
    }, {
      ...
      location: "Nice",
      region: "Provence-Alpes-C\xF4te d'Azur"
    }, {
    ...
    var coastalDestinations = [{
      ...
      location: "Playa de las Am\xE9ricas",
      region: "Canary Islands"
    }, {
      location: "Torquay",
      region: "England"
    ...
    function generateDestinationList(destinationsArray, classPrefix) {
      var $destinationTypeContainer = $('<div></div>', {
        'class': "".concat(classPrefix, "-destinations-content-container list-of-destinations-content-container")
      });
      var $destinationContentContainer = $('.list-of-destinations-section');
      $destinationTypeContainer.appendTo($destinationContentContainer);
      destinationsArray.forEach(function (destination) {
        $destinationTypeContainer.append("<div class=\"destination-container\">\n            <h3 class=\"destination-location\">".concat(destination.location, "</h3>\n            <h4 class=\"destination-country\">").concat(destination.region, "</h4>\n          </div>"));
      });
      $(".".concat(classPrefix, "-destination-title")).on('click', function () {
